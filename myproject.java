// Author -- vaishnavimovva
import javax.imageio.ImageIO;                  // provides a set of static methods for reading and writing images
import javax.imageio.stream.ImageInputStream;
import javax.swing.*;
import java.awt.*;  // Abstract Window Toolkit -  creating and managing graphical user interface components, such as buttons, labels, and panels.
import java.awt.event.*;  //handling events in a GUI - such as button clicks or mouse movements.
import java.io.*; 
import java.util.*;
import java.util.stream.Collectors; // collecting elements into a collection.
import javax.swing.Timer; // schedule a task to be performed repeatedly at a fixed interval.
class Game {
    public static class Controller {
        ...
        public Controller(Model model) {
            this.window = new JFrame("Memory"); // Create the window Title "Memory"
            ...
            this.window.setResizable(false); // Disable the resizing of the window- window size remains fixed
            this.reset(model); // Reset the game
        }
        // Reset the game
        public void reset(Model model) {
            this.model = model; // It is used to hold the reference to the game model, which represents the underlying data and logic of the game.
            this.view = new View(model);
            this.window.setVisible(false); // Sets the visibility of the window to false initially, so it won't be shown until later.
            this.window.setContentPane(view); //content pane is the main area where you can add and arrange 
            this.window.pack(); //  Causes the window to resize itself to fit the preferred size and layout of its components.
            this.window.setLocationRelativeTo(null); // Centers the window on the screen.
            for (JButton button : this.model.getButtons()) {
                
                button.addActionListener(new ButtonActionListener(this)); //This listener will handle button actions in the game.
            }
    ...
    public static class Model {
        ...
        static final String[] AVAILABLE_IMAGES = new String[]{"pic 1.jpeg", "pic 2.jpeg", "pic 3.jpeg", "pic 4.jpeg", "pic 5.jpeg", "pic 6.jpeg", "pic 7.jpeg", "pic 9.jpeg", "pic 9.jpeg"};
        ...
        public Model(int columns) {
            ...
            for (int i = 0; i < numberOfImage - numberOfImage % 2; i++) { // This ensures that each image is added twice, except when the total number of images is odd.
            ...
            for (int i = 0; i < numberOfImage; i++) { // loop iterates numberOfImage times to create buttons for the game
        ...
        public boolean isGameStarted() {
            return this.gameStarted; //It allows external classes to check the game state 
        }
        // start the game
    ...
    public static class MemoryButton extends JButton {
        ...
        static final Image NO_IMAGE = Utilities.loadImage("no image.jpeg");
    ...
    public static class ButtonActionListener implements ActionListener {
        ...
        static final Image TRAP_IMAGE = Utilities.loadImage("no image.jpeg");
        ...
        public ButtonActionListener(Controller controller) {
            ...
            this.trap = new ReferencedIcon(TRAP_IMAGE, "no image.jpeg");
        }
        // Method to handle the button clicks and check if two images are same -- author - vaishanvimovva
        @Override
